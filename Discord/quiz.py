...
class Quiz:
    ...
    def __init__(self, client, win_limit=10, hint_time=30):
        ...
        datafiles = os.listdir('C:/Users/borst/OneDrive/Desktop/discordQuizBot/quizdata')
        for df in datafiles:
            filepath = 'C:/Users/borst/OneDrive/Desktop/discordQuizBot/quizdata/questions.trivia.en'
    ...
    async def _hint(self, hint_question, hint_number):
        ...
        if self.__running and self.current_question is not None:
            ...
            if (self.current_question == hint_question 
                ...
                await self._channel.send('Hint {}: {}'.format(hint_number, hint), tts=True)
                if hint_number < 5:
    ...
    async def start(self, channel):
        ...
        if self.__running:
            ...
            await channel.send('@everyone Quiz starting in 10 seconds...', tts=True)
    ...
    async def stop(self):
        ...
        if self.__running:
            #print results
            #stop quiz
            await channel.send(self._channel, 'Quiz stopping.', tts=True)
            if(self.current_question is not None):
                await channel.send(self._channel, 
            ...
            await channel.send(self._channel, 'No quiz running, start one with !ask or !quiz', tts=True)
        ...
        if self.__running and self.current_question is not None:
            ...
            if self.current_question.answer_correct(message.content):
                ...
                if self.scores[message.author.name] == self._win_limit:
                    ...
                    await self.print_scores()
                    await channel.send(self._channel, '{} has won! Congratulations.'.format(message.author.name), tts=True)
                    self._questions.append(self._asked)
                    self._asked = []
                    self.__running = False                    
